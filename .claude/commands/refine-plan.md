...
4. **Record the decision — don't write the file yet.** Once the user decides, note the chosen answer and its rationale in your working notes (and any forced downstream item edit it implies). The file edits are applied in one consolidated pass at the end of the walk (see below) — this avoids N separate file writes for N questions, which is the cheaper path.

**Shortcut signals.** If the user says "accept your recommendations", "pick the best long-term for all", or similar mid-resolution, stop the per-question `AskUserQuestion` prompts and **batch-resolve the remaining questions** using your recommended answer for each (see `feedback_autonomous_best_long_term` in memory). Then go straight to the apply pass.

**Apply all resolutions in one pass (end of walk).** Once every question is answered, make a single consolidated edit that: moves all resolved questions into `## Resolved decisions` (create the section if absent), clears `## Open questions` and removes the now-empty heading, removes any inline `TODO`/`Deferred`/"decide during" markers the questions came from, and applies any forced downstream item edits (where an item contradicts a choice). One write for the whole batch, not one per question.

**Safety valve — flush early if context is at risk.** Because decisions live only in your working notes until the end-of-walk write, a long or interrupted walk could lose them. So: if the walk spans many turns and you receive a context-compaction/summary notice, or the user steps away mid-walk, **flush the decisions resolved so far to the file immediately** (apply the pass above for the answered questions), then continue with the rest. Don't carry a large backlog of unwritten decisions across a compaction boundary.
...
- **Batch the writes at the end, not per question.** Hold each resolution in your working notes during the walk, then apply them all in one consolidated file edit at the end — fewer tool round-trips, cheaper. The one exception is the safety valve: flush decided questions to the file early if a long/interrupted walk or a context compaction threatens to lose them.
